// src/Walk.jsx — v4: Ihsan-grade redesign
// Features: auto-login, retroactive editing (3 days back), companion discovery + spouse support, full visual overhaul
...
// Get date object from a YYYY-MM-DD string
...
// Get navigable dates (today + 3 days back)
...
function sc(d) {
  ...
  if (d.adhkar_sabah) s += 2;
  if (d.adhkar_masa) s += 2;
  const q = QO.find(o => o.value === d.quran);
  if (q) s += Math.min(q.pts, 5);
...
.wi{width:100%;padding:16px 20px;border-radius:14px;border:1.5px solid rgba(255,255,255,0.05);background:rgba(255,255,255,0.03);color:#fff;font-size:15px;font-family:'Outfit',sans-serif;outline:none;transition:border-color 0.25s,box-shadow 0.25s,background 0.25s}
...
.btn-gold{width:100%;padding:16px;border-radius:14px;border:none;background:linear-gradient(135deg,#D4A853 0%,#b8923d 100%);color:#0a0a16;font-size:16px;font-weight:700;letter-spacing:0.3px;font-family:'Outfit',sans-serif;cursor:pointer;transition:all 0.25s;position:relative;overflow:hidden}
.btn-gold::after{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(255,255,255,0.15) 0%,transparent 60%);opacity:0;transition:opacity 0.25s}
.btn-gold:hover::after{opacity:1}
.btn-gold:hover{transform:translateY(-1px);box-shadow:0 8px 32px rgba(212,168,83,0.25)}
.btn-gold:active{transform:scale(0.98)}.btn-gold:disabled{opacity:0.5;cursor:wait;transform:none;box-shadow:none}
...
.dash-wrap{min-height:100vh;max-width:500px;margin:0 auto;font-family:'Outfit',sans-serif;color:${C.text};background:linear-gradient(180deg,${C.bg} 0%,${C.bg2} 30%,#0f172a 100%);position:relative;overflow:hidden}
.p-btn{border:1.5px solid ${C.bdr};cursor:pointer;font-family:'Outfit',sans-serif;display:flex;align-items:center;justify-content:space-between;width:100%;padding:15px 18px;border-radius:16px;background:rgba(255,255,255,0.015);margin-bottom:8px;transition:all 0.2s;-webkit-tap-highlight-color:transparent}
.p-btn:active{transform:scale(0.975);transition:transform 0.1s}
.c-btn{border:1.5px solid ${C.bdr};cursor:pointer;font-family:'Outfit',sans-serif;display:flex;align-items:center;gap:12px;width:100%;padding:14px 18px;border-radius:16px;background:rgba(255,255,255,0.015);margin-bottom:8px;transition:all 0.2s;-webkit-tap-highlight-color:transparent}
.c-btn:active{transform:scale(0.975);transition:transform 0.1s}
.date-nav-btn{border:none;cursor:pointer;width:40px;height:40px;border-radius:12px;background:rgba(255,255,255,0.03);display:flex;align-items:center;justify-content:center;color:${C.dim};font-size:16px;font-family:'Outfit',sans-serif;transition:all 0.2s;-webkit-tap-highlight-color:transparent}
.date-nav-btn:hover{background:rgba(212,168,83,0.08);color:${C.gold}}
.date-nav-btn:active{transform:scale(0.92)}
.date-nav-btn:disabled{opacity:0.2;cursor:default;transform:none}
.settings-menu{position:absolute;top:60px;right:20px;width:220px;background:rgba(15,15,30,0.95);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:6px;z-index:50;animation:slideDown 0.2s ease}
...
function Particles() {
  const dots = useRef(Array.from({ length: 18 }, () => ({
    w: 1 + Math.random() * 2.5,
...
function PrayerRow({ prayer, value, onCycle, jumu }) {
  ...
  return (
    <button onClick={onCycle} className="p-btn" style={{
      borderColor: m ? C.greenBdr : p ? "rgba(212,168,83,0.15)" : undefined,
      background: m ? "rgba(52,211,153,0.06)" : p ? "rgba(212,168,83,0.06)" : undefined,
    }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        <div style={{
          width: 40, height: 40, borderRadius: 12,
          background: m ? "rgba(52,211,153,0.1)" : p ? "rgba(212,168,83,0.1)" : "rgba(255,255,255,0.04)",
          display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17,
          border: `1.5px solid ${m ? "rgba(52,211,153,0.15)" : p ? "rgba(212,168,83,0.12)" : "rgba(255,255,255,0.04)"}`,
          transition: "all 0.2s",
        }}>{m ? "🕌" : p ? "✓" : prayer.icon}</div>
        <div style={{ textAlign: "left" }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: m ? C.green : p ? C.gold : C.text }}>{label}</div>
      ...
      <span style={{
        ...
        background: active ? "rgba(0,0,0,0.2)" : "rgba(255,255,255,0.02)",
...
function CheckRow({ label, icon, checked, onToggle }) {
  return (
    <button onClick={onToggle} className="c-btn" style={{
      borderColor: checked ? C.greenBdr : undefined,
      background: checked ? "rgba(52,211,153,0.05)" : undefined,
    }}>
      <div style={{
        width: 22, height: 22, borderRadius: 7,
        border: `2px solid ${checked ? C.green : "rgba(255,255,255,0.08)"}`,
        background: checked ? C.green : "transparent",
        display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
        transition: "all 0.15s",
      }}>{checked && <span style={{ color: C.bg, fontSize: 12, fontWeight: 800 }}>✓</span>}</div>
      <span style={{ fontSize: 15 }}>{icon}</span>
      <span style={{ fontSize: 14, fontWeight: 500, color: checked ? C.green : C.text, opacity: checked ? 0.8 : 1 }}>{label}</span>
    </button>
  );
}
...
export default function Walk() {
  const [scr, setScr] = useState("loading"); // start with loading for auto-login
  ...
  const [partnerLabel, setPartnerLabel] = useState("Brother"); // "Brother" or "Spouse"
  ...
  const [pairStep, setPairStep] = useState("intro"); // intro, manual, invite
  const [companionType, setCompanionType] = useState("brother"); // brother, spouse
  ...
  const load = useCallback(async (id) => {
    try {
      ...
      if (ps.length > 0) {
        ...
        // Check companion_type if it exists
        if (ps[0].companion_type) setPartnerLabel(ps[0].companion_type === "spouse" ? "Spouse" : "Brother");
  ...
  useEffect(() => {
    async function autoLogin() {
      ...
      try {
        ...
        setUid(pr[0].id);
        setUser(pr[0].username);
        await load(pr[0].id);
        // Check if they have a partner
    ...
    try {
      if (aMode === "signup") {
        ...
        if (cr && cr.length > 0) {
          ...
          localStorage.setItem("nurUser", u);
        }
      } else {
        ...
        localStorage.setItem("nurUser", u);
        const ps = await sb(`partnerships?or=(user_a.eq.${pr[0].id},user_b.eq.${pr[0].id})`) || [];
        if (ps.length > 0) { await load(pr[0].id); setScr("dashboard"); }
        else setScr("pair");
      }
  ...
  async function pair() {
    ...
    try {
      ...
      const ct = companionType === "spouse" ? "spouse" : "brother";
      await sb("partnerships", "POST", { user_a: uid, user_b: pr[0].id, companion_type: ct });
      setPartner(p);
      setPartnerLabel(ct === "spouse" ? "Spouse" : "Brother");
      await load(uid);
      setScr("dashboard");
  ...
  function upd(key, val) {
    if (isToday) {
      setTd(prev => {
        const n = { ...prev };
        if (val === null || val === false || val === undefined) delete n[key];
        else n[key] = val;
        save(n, today);
        return n;
      });
      ...
      setWk(prev => ({
        ...prev,
        [selectedDate]: { ...(prev[selectedDate] || {}), ...n, date: selectedDate, user_id: uid },
      }));
    }
  }
  ...
  function cyc(id) {
    ...
    const v = data[id];
    if (!v) upd(id, "prayed");
    else if (v === "prayed") upd(id, "masjid");
    else upd(id, null);
  }
  ...
  function navDate(dir) {
    ...
    if (newIdx >= 0 && newIdx < navigableDates.length) {
      setSelectedDate(navigableDates[newIdx]);
    }
  }
        ...
        <div style={{ maxWidth: 360, textAlign: "center" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center", marginTop: 28, animation: "fadeUp 0.6s ease 0.3s both" }}>
            {["5 Daily Prayers", "Quran Log", "Adhkar", "Sunnah Tracker", "1-on-1 Companion", "Streak System"].map((t, i) => (
              <span key={i} style={{ fontSize: 11, fontWeight: 600, color: C.dim, padding: "6px 14px", borderRadius: 20, background: "rgba(255,255,255,0.03)", border: `1px solid ${C.bdr}` }}>{t}</span>
            ))}
          </div>
          <div style={{ marginTop: 36, padding: "0 20px", animation: "fadeUp 0.6s ease 0.4s both" }}>
            <p style={{ fontSize: 13, color: "rgba(212,168,83,0.5)", fontFamily: "'Playfair Display', serif", fontStyle: "italic", lineHeight: 1.7 }}>
      ...
      <div className="auth-right">
        <div style={{ width: "100%", maxWidth: 320 }}>
          ...
          <div style={{ display: "flex", gap: 0, marginBottom: 24, background: "rgba(255,255,255,0.03)", borderRadius: 14, padding: 3, border: `1px solid ${C.bdr}`, animation: "fadeUp 0.5s ease 0.2s both" }}>
            {["login", "signup"].map(m => (
              <button key={m} onClick={() => { setAMode(m); setAErr(""); }} style={{
          ...
          {aErr && <div style={{ fontSize: 13, color: "#f87171", marginBottom: 14, textAlign: "center", padding: "10px 14px", background: "rgba(248,113,113,0.05)", borderRadius: 12, border: "1px solid rgba(248,113,113,0.1)", animation: "slideDown 0.2s ease" }}>{aErr}</div>}
  ...
  // ── PAIR (redesigned companion discovery) ──
  if (scr === "pair") return (
    <div style={{
      ...
      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 380 }}>
        ...
        {/* Step: Intro — what is a companion? */}
        {pairStep === "intro" && (
          <div style={{ textAlign: "center", animation: "fadeUp 0.5s ease both" }}>
            ...
            <p style={{ fontSize: 14, color: C.sub, lineHeight: 1.7, marginBottom: 8 }}>
              A companion is one person — a brother or your spouse — who walks this path with you.
            </p>

            <div style={{
              background: "rgba(212,168,83,0.04)", border: `1px solid rgba(212,168,83,0.12)`,
              borderRadius: 16, padding: "20px 18px", marginTop: 20, marginBottom: 28, textAlign: "left",
            }}>
            ...
            <div style={{ marginBottom: 24 }}>
              <div style={{ fontSize: 10, letterSpacing: 2, textTransform: "uppercase", color: C.dim, fontWeight: 700, marginBottom: 10 }}>
                Who will you walk with?
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                {[
                  { id: "brother", label: "A Brother", icon: "👤" },
                  { id: "spouse", label: "My Spouse", icon: "💍" },
                ].map(opt => (
                  <button key={opt.id} onClick={() => setCompanionType(opt.id)} style={{
                    flex: 1, padding: "14px 12px", borderRadius: 14,
                    border: `1.5px solid ${companionType === opt.id ? "rgba(212,168,83,0.3)" : C.bdr}`,
                    background: companionType === opt.id ? "rgba(212,168,83,0.06)" : "rgba(255,255,255,0.02)",
            ...
            <button onClick={() => setPairStep("manual")} className="btn-gold" style={{ marginBottom: 10 }}>
              I know their username
            </button>
            <button onClick={() => setPairStep("invite")} className="btn-outline" style={{ marginBottom: 10 }}>
              Send them an invite link
            </button>
            <button onClick={async () => { await load(uid); setScr("dashboard"); }} className="btn-ghost">
              Walk alone for now
            </button>
          </div>
        )}

        {/* Step: Manual username entry */}
        {pairStep === "manual" && (
          <div style={{ textAlign: "center", animation: "fadeUp 0.4s ease both" }}>
            <button onClick={() => setPairStep("intro")} style={{
              border: "none", background: "none", color: C.dim, fontSize: 13, cursor: "pointer",
              fontFamily: "'Outfit',sans-serif", marginBottom: 20, display: "flex", alignItems: "center", gap: 6,
            }}>← Back</button>
            ...
            <p style={{ fontSize: 13, color: C.sub, marginBottom: 24, lineHeight: 1.6 }}>
              Your {companionType === "spouse" ? "spouse" : "brother"} needs to have signed up already.
            </p>
            <input
              value={pInput} onChange={e => setPInput(e.target.value)}
              placeholder={companionType === "spouse" ? "Spouse's username" : "Brother's username"}
              className="wi"
              onKeyDown={e => { if (e.key === "Enter") pair(); }}
              style={{ marginBottom: 14, textAlign: "center" }}
            />
            {pErr && <div style={{ fontSize: 13, color: "#f87171", marginBottom: 12, textAlign: "center", padding: "10px 14px", background: "rgba(248,113,113,0.05)", borderRadius: 12, border: "1px solid rgba(248,113,113,0.1)" }}>{pErr}</div>}
            <button onClick={pair} disabled={busy} className="btn-gold" style={{ marginBottom: 10 }}>
              {busy ? "..." : "Pair Up"}
            </button>
          </div>
        )}
        ...
        {pairStep === "invite" && (
          <div style={{ textAlign: "center", animation: "fadeUp 0.4s ease both" }}>
            ...
            <h2 style={{ fontSize: 22, fontWeight: 800, color: "#fff", marginBottom: 8 }}>
              Invite Your {companionType === "spouse" ? "Spouse" : "Brother"}
            </h2>
            <p style={{ fontSize: 13, color: C.sub, marginBottom: 24, lineHeight: 1.6 }}>
              Send them this message. Once they sign up, come back here and enter their username.
            </p>
            <div style={{
              background: "rgba(255,255,255,0.03)", border: `1px solid ${C.bdr}`,
              borderRadius: 16, padding: "18px 16px", marginBottom: 20, textAlign: "left",
            }}>
            ...
            <button
              ...
              id="copy-invite"
              className="btn-gold" style={{ marginBottom: 10 }}
            >Copy Message</button>
            <button onClick={() => setPairStep("manual")} className="btn-outline" style={{ marginBottom: 10 }}>
              I have their username now
            </button>
            <button onClick={async () => { await load(uid); setScr("dashboard"); }} className="btn-ghost">
              Skip for now
            </button>
  ...
  return (
    <div className="dash-wrap">
      ...
      {/* Header */}
      <div style={{ padding: "22px 20px 14px", display: "flex", justifyContent: "space-between", alignItems: "flex-start", position: "relative", zIndex: 10 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: C.dim, marginTop: 4 }}>
            {isFri && <span style={{ color: C.gold, fontWeight: 600 }}>Jumu'ah Mubarak · </span>}
            {today}
          </div>
        ...
        <div ref={settingsRef} style={{ position: "relative" }}>
          <button onClick={() => setSettings(!settings)} style={{
            border: `1.5px solid ${settings ? "rgba(212,168,83,0.2)" : C.bdr}`, cursor: "pointer",
            fontFamily: "'Outfit',sans-serif", width: 38, height: 38, borderRadius: 12,
            background: settings ? "rgba(212,168,83,0.06)" : "rgba(255,255,255,0.02)",
            color: settings ? C.gold : C.dim, fontSize: 14,
          ...
          {settings && (
            <div className="settings-menu">
              ...
              {partner && (
                <div style={{ padding: "0 14px 8px", fontSize: 11, color: C.dim }}>
                  Paired with <span style={{ color: C.teal }}>{partner}</span>
                </div>
              )}
              ...
              <button onClick={() => {
                setUser(""); setUid(""); setScr("auth"); setSettings(false);
                setTd({}); setWk({}); setPartner("");
                setSelectedDate(tk());
                localStorage.removeItem("nurUser");
      ...
      <div style={{ padding: "0 20px 16px", position: "relative", zIndex: 1 }}>
        <div style={{
          ...
          borderRadius: 14,
          border: !isToday ? "1px solid rgba(212,168,83,0.1)" : "1px solid transparent",
          transition: "all 0.3s",
        }}>
          <button className="date-nav-btn" onClick={() => navDate("prev")} disabled={!canGoBack}>‹</button>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: isToday ? C.text : C.gold }}>
      ...
      <div style={{ padding: "0 20px 24px", position: "relative", zIndex: 1 }}>
        <div style={{
          ...
          background: "linear-gradient(135deg, rgba(255,255,255,0.02) 0%, rgba(212,168,83,0.02) 100%)",
          borderRadius: 20,
          border: `1px solid ${score.pct === 100 ? "rgba(52,211,153,0.15)" : C.bdr}`,
          ...
          <div style={{
            ...
            width: `${score.pct}%`,
            background: score.pct === 100
              ? "linear-gradient(90deg, #34D399, #4ECDC4)"
              : `linear-gradient(90deg, ${C.gold}, ${C.goldMid})`,
            borderRadius: "0 3px 0 0",
            transition: "width 0.5s ease",
            ...(score.pct > 0 && score.pct < 100 ? { animation: "progressPulse 2s ease infinite" } : {}),
          }} />
          {/* Subtle shimmer overlay on 100% */}
          {score.pct === 100 && <div style={{
            position: "absolute", inset: 0, borderRadius: 20,
            background: "linear-gradient(135deg, rgba(52,211,153,0.03) 0%, transparent 50%, rgba(78,205,196,0.03) 100%)",
            pointerEvents: "none",
          }} />}
          <div>
            <div style={{
              fontSize: 60, fontWeight: 900, lineHeight: 1,
              color: pctColor, letterSpacing: "-3px",
              fontFamily: "'Outfit', sans-serif",
            }}>
              {score.pct}<span style={{ fontSize: 24, fontWeight: 600, color: C.dim, letterSpacing: 0 }}>%</span>
            </div>
            <div style={{ marginTop: 12 }}><Badge l={level.l} n={level.n} /></div>
          </div>
          <div style={{ textAlign: "right", paddingBottom: 6 }}>
            <div style={{ fontSize: 38, fontWeight: 900, color: streak > 0 ? C.gold : C.muted, lineHeight: 1, letterSpacing: "-1px" }}>{streak}</div>
            <div style={{ fontSize: 9, color: C.dim, letterSpacing: 2, textTransform: "uppercase", marginTop: 5 }}>day streak</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: "0 20px 24px", position: "relative", zIndex: 1 }}>
        <div style={{
          ...
          {d7.map(d => {
            ...
            const l = lv(dd);
            const it = k === today;
            const isSelected = k === selectedDate && !isToday;
            let bg = "rgba(255,255,255,0.03)";
            if (s.pct > 0 && s.pct < 40) bg = "rgba(212,168,83,0.1)";
            if (s.pct >= 40 && s.pct < 75) bg = "rgba(212,168,83,0.22)";
            if (s.pct >= 75) bg = "rgba(52,211,153,0.18)";
            if (l.l >= 3) bg = "rgba(167,139,250,0.22)";
            return (
              <div key={k} style={{ flex: 1, textAlign: "center" }}>
                <div style={{
                  aspectRatio: "1", borderRadius: 10,
                  background: bg,
                  border: isSelected ? `2px solid ${C.teal}` : it ? `2px solid ${C.gold}` : "2px solid transparent",
                ...
                <div style={{
                  ...
                  fontWeight: it ? 700 : 400,
                  color: isSelected ? C.teal : it ? C.gold : C.muted,
                }}>{dayLbl(d)}</div>
              </div>
            );
          })}
      ...
      {partner && (
        <div style={{ padding: "0 20px 24px", position: "relative", zIndex: 1 }}>
          <div style={{
            ...
            background: "linear-gradient(135deg, rgba(212,168,83,0.03) 0%, rgba(78,205,196,0.02) 100%)",
            border: "1px solid rgba(212,168,83,0.08)",
          }}>
            <div style={{ fontSize: 9, letterSpacing: 2, textTransform: "uppercase", color: C.gold, marginBottom: 14, fontWeight: 700 }}>
              {partnerLabel === "Spouse" ? "Your Spouse" : "Your Companion"}
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{
                width: 44, height: 44, borderRadius: 14,
                background: pIn ? "rgba(52,211,153,0.08)" : "rgba(255,255,255,0.03)",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 17, fontWeight: 700,
                color: pIn ? C.green : C.muted,
                border: `2px solid ${pIn ? "rgba(52,211,153,0.15)" : C.bdr}`,
              ...
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 22, fontWeight: 900, color: pIn ? C.green : C.muted }}>{pPct}%</div>
                <div style={{ fontSize: 10, color: C.dim }}>{pStr}d streak</div>
              </div>
            </div>
            {pIn && <div style={{ marginTop: 12 }}><Badge l={pLv} n={["Starting", "Foundation", "Growth", "Elite"][pLv]} /></div>}
          </div>
        </div>
      )}

      {/* Today's Flow (or selected day) */}
      <div style={{ padding: "0 20px 28px", position: "relative", zIndex: 1 }}>
        ...
        <div style={{
          ...
          background: currentData.quran ? "rgba(52,211,153,0.05)" : "rgba(255,255,255,0.015)",
          border: `1.5px solid ${currentData.quran ? C.greenBdr : C.bdr}`,
          ...
          <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
            {QO.map(o => (
              <button key={o.value} onClick={() => upd("quran", currentData.quran === o.value ? null : o.value)} style={{
                border: `1.5px solid ${currentData.quran === o.value ? C.green : "rgba(255,255,255,0.05)"}`,
                cursor: "pointer", fontFamily: "'Outfit',sans-serif",
                padding: "9px 16px", borderRadius: 10,
                fontSize: 12, fontWeight: 600,
                background: currentData.quran === o.value ? "rgba(52,211,153,0.12)" : "rgba(255,255,255,0.02)",
      ...
      {/* Level Achievement */}
      {level.l >= 1 && (
        <div style={{ textAlign: "center", padding: "20px 20px 8px", position: "relative", zIndex: 1 }}>
          <div style={{ fontSize: 38, marginBottom: 8 }}>{level.l === 3 ? "👑" : level.l === 2 ? "🔥" : "✅"}</div>
          <div style={{ fontSize: 17, fontWeight: 700, color: level.l === 3 ? C.purple : level.l === 2 ? C.teal : C.green }}>
            Level {level.l} — {level.n}
          </div>
          <div style={{ fontSize: 12, color: C.dim, marginTop: 4, fontStyle: "italic", fontFamily: "'Playfair Display', serif" }}>
      ...
      {/* Footer */}
      <div style={{ textAlign: "center", padding: "32px 20px 40px", position: "relative", zIndex: 1 }}>
        <div style={{ width: 40, height: 1, background: "rgba(212,168,83,0.15)", margin: "0 auto 14px" }} />
